...
install_feroxbuster() {
    ...
    # coba via apt dulu
    if ! apt_install_or_fix feroxbuster; then
        echo -e "${R}[!] feroxbuster tidak tersedia di repo, silakan install via cargo manual kalau perlu.${N}"
    fi
}
...
install_by_id() {
    case "$1" in
        ...
        56) install_gf ;;
        57) install_gau ;;
        58) install_waybackurls ;;
        59) install_anew ;;
        60) install_paramspider ;;
        61) install_feroxbuster ;;
...
while true; do
    ...
    echo    "  7) urlfinder         29) wapiti              61) feroxbuster"
    echo    "  8) asnmap            30) nikto"
    echo    "  9) chaos-client      31) gobuster"
